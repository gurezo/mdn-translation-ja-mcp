...
翻訳作業では **`translated-content` リポジトリだけをワークスペースとして開く**運用がよくあります。次を満たすと、環境変数なしでも MCP が兄弟の `content` を解決できます。
...
親ディレクトリ構成が異なる場合は、MCP 設定の `env` に `MDN_CONTENT_ROOT` と `MDN_TRANSLATED_CONTENT_ROOT` を**両方**指定してください（[examples/cursor-mcp-with-env.json](examples/cursor-mcp-with-env.json)）。
