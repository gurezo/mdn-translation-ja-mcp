...
1. **ランタイム**  
   [Node.js](https://nodejs.org/) 22 以上（LTS 推奨）、[Cursor](https://cursor.com/)（MCP の **stdio** または **Streamable HTTP** に対応した版）。HTTP 必須ではありません。

2. **リポジトリの配置**  
   上記のディレクトリツリーのとおり、`content`・`translated-content`・本リポジトリを**同じ親ディレクトリ**に並べることを推奨します。別の場所に置く場合は、次項の環境変数で**両方**を指定してください。
...
4. **（任意）環境変数**  
   ...
   `content` / `translated-content` を別の場所に置く場合は、stdio 利用時は [examples/cursor-mcp-with-env.json](examples/cursor-mcp-with-env.json) のとおり MCP 設定の `env` に `MDN_CONTENT_ROOT` と `MDN_TRANSLATED_CONTENT_ROOT` を**両方**書く（片方だけは不可。後述「パスの解決順」）。  
...
本サーバーが提供する MCP ツール名（Cursor のツール一覧に表示される名前）と主な用途です。**先頭に `/` は付きません**（MCP の登録名は `mdn_trans_start` のようなスネークケースです）。
...
| MCP ツール名 | 主な用途 |
| --- | --- |
| `translation_rules` | 表記・L10N・用語集・文体などの参照リンクとローカルルール JSON |
...
| `mdn_trans_commit_get` | `content` 側のコミットハッシュ取得・`sourceCommit` のたたき台 |
...
`mdn_workspace_paths` および各ツールのワークスペース解決で共通です。環境変数の要否は「導入手順（チェックリスト）」の（任意）環境変数と同じ前提です。

1. **環境変数（任意）** — `MDN_CONTENT_ROOT` と `MDN_TRANSLATED_CONTENT_ROOT` を**両方**指定すると、その絶対パスをそのまま使います。片方だけは不可（誤設定防止のためエラー）。
...
環境変数の MCP 設定例は [examples/cursor-mcp-with-env.json](examples/cursor-mcp-with-env.json) を参照してください。
...
| 症状 | 確認すること |
| --- | --- |
| HTTP で Cursor が MCP に接続できない | **`npm run start:http` が起動しているか**<br>Cursor の MCP 設定の `url`（ホスト・ポート・パス `/mcp`）が一致しているか。 |
