...
2. **`translated-content/.cursor/mcp.json`** として [examples/translated-content-cursor-mcp-example.json](examples/translated-content-cursor-mcp-example.json) を参考に `mcpServers` を記述します（ワークスペースのルートが `translated-content` のとき、このパスに置きます）。
...
MCP の登録は Cursor の **`mcp.json`** で行います。`translated-content` をワークスペースに開く場合は **`translated-content/.cursor/mcp.json`**（上記）を使います。その他のワークスペースでは Cursor のユーザー設定側の `mcp.json` になることがあります。
...
（MCP は **`translated-content/.cursor/mcp.json`**。手順と「翻訳 PR に `mcp.json` を含めない」旨の注意は「translated-content リポジトリ側で行うこと」を参照してください。）
