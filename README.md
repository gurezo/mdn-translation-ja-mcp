...
4. **レビュー** — **`mdn_trans_review`** で禁止・注意表現リストに基づく簡易チェックを行います（詳細はエージェントが [表記ガイドライン](https://github.com/mozilla-japan/translation/wiki/Editorial-Guideline) 等を参照してください）。
...
- 「この翻訳をルールでざっと確認して」→ `mdn_trans_review`（対象ファイルパスを指定）。
