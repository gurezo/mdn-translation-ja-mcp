...
- （任意 `--with-rules`）`rules/01-mdn-mcp-tools.mdc`（`integrations/cursor/rules/` からコピー）

`.agents/skills` と workflow Skill、`00-mdn-translation.mdc` はコピーしない。README では Rules / Skills を「任意」と案内する。

Cursor 向け optional セットアップ。他 MCP クライアントでは不要。最低限はサーバー登録だけである。
...
| `rules/01-mdn-mcp-tools.mdc` | setup が `--with-rules` のときコピーする Rule。本リポジトリの `.cursor/rules/01-mdn-mcp-tools.mdc` より短い（CLI フォールバックと workspace 節が簡略） |
...
| `integrations/cursor/rules/01-mdn-mcp-tools.mdc`                  | 上記の短縮版。setup が `--with-rules` のときこちらをコピー |
